...
const AccountsPage = () => {
  ...
  const fetchAccounts = async () => {
    try {
      const response = await axios.get("http://0.0.0.0:5000/get_accounts");
  ...
  const handleAddAccount = async () => {
    try {
      const response = await axios.post(
        "http://0.0.0.0:5000/create_account",
        newAccount
      );
  ...
  const handleUpdateAccount = async () => {
    try {
      const response = await axios.post(
        `http://0.0.0.0:5000/update_account/${editAccountId}`,
        newAccount
      );
      if (response.status === 200) {
  ...
  const handleDeleteAccount = async (accountId) => {
    try {
      const response = await axios.post(
        `http://0.0.0.0:5000/delete_account/${accountId}`
      );
      if (response.status === 200) {
